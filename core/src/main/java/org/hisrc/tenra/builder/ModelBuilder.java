...
public class ModelBuilder {
	...
	public void add(Object value) {
		if (value instanceof RailwayLinkType) {
			...
			throw new IllegalArgumentException(MessageFormat.format(
					"Unsupported class [{0}], value [{1}].", value.getClass(),
					value));
		}
	}
	...
	private void addRailwayNode(RailwayNodeType value) {
		final RailwayNode railwayNode = RailwayNodeTypeConverter.INSTANCE
				.convert(value);
	...
	private void addRailwayLink(RailwayLinkType value) {
		final RailwayLink railwayLink = RailwayLinkTypeConverter.INSTANCE
				.convert(value);
	...
	private void addRailwayStationNode(RailwayStationNodeType value) {
		final RailwayStationNode railwayStationNode = RailwayStationNodeTypeConverter.INSTANCE
				.convert(value);
		railwayStationNodes.put(railwayStationNode.getId(), railwayStationNode);
	}
	...
	private void addRailwayStationCode(RailwayStationCodeType value) {
		final RailwayStationCode railwayStationCode = RailwayStationCodeTypeConverter.INSTANCE
				.convert(value);
	...
	private void addRailwayLine(RailwayLineType value) {
		final RailwayLine railwayLine = RailwayLineTypeConverter.INSTANCE
				.convert(value);
	...
	private void addMarkerPost(MarkerPostType value) {
		final MarkerPost markerPost = MarkerPostTypeConverter.INSTANCE
				.convert(value);
		// TODO
	...
	private void addRailwayLinkSequence(RailwayLinkSequenceType value) {
		final RailwayLinkSequence railwayLinkSequence = RailwayLinkSequenceTypeConverter.INSTANCE
				.convert(value);
		railwayLinkSequences.put(railwayLinkSequence.getId(),
				railwayLinkSequence);
	...
	private void buildRailwayStationCodes() {
		for (RailwayStationCode railwayStationCode : this.railwayStationCodes
				.values()) {
			final String railwayStationNodeId = railwayStationCode
					.getRailwayStationNodeId();
			final RailwayStationNode railwayStationNode = Ensure
					.mapContainsKey(this.railwayStationNodes,
							railwayStationNodeId, "railwayStationNodes");
			if (railwayStationNode.getRailwayStationCode() != null) {
				Ensure.propertyIsNull(
						railwayStationNode.getRailwayStationCode(),
						railwayStationNode, "railwayStationCode");
			}
	...
	private void buildRailwayNodes() {
		for (RailwayNode railwayNode : this.railwayNodes.values()) {
			final List<RailwayLink> spokeStarts = new ArrayList<>(railwayNode
					.getSpokeStartIds().size());
			final List<RailwayLink> spokeEnds = new ArrayList<>(railwayNode
					.getSpokeEndIds().size());
			for (String spokeStartId : railwayNode.getSpokeStartIds()) {
				Validate.notNull(spokeStartId);
				final RailwayLink spokeStart = Ensure.mapContainsKey(
						this.railwayLinks, spokeStartId, "railwayNodes");
				spokeStarts.add(spokeStart);

			}
			for (String spokeEndId : railwayNode.getSpokeEndIds()) {
				Validate.notNull(spokeEndId);
				final RailwayLink spokeEnd = Ensure.mapContainsKey(
						this.railwayLinks, spokeEndId, "railwayLinks");
	...
	private void buildRailwayStationNodes() {
		for (RailwayStationNode railwayStationNode : this.railwayStationNodes
				.values()) {
			final List<RailwayLink> spokeStarts = new ArrayList<>(
					railwayStationNode.getSpokeStartIds().size());
			final List<RailwayLink> spokeEnds = new ArrayList<>(
					railwayStationNode.getSpokeEndIds().size());
	...
	private void buildRailwayLinks() {
		for (RailwayLink railwayLink : this.railwayLinks.values()) {
			final RailwayNode startNode = Ensure.mapContainsKey(
					this.railwayNodes, railwayLink.getStartNodeId(),
					"railwayNodes");
			final RailwayNode endNode = Ensure.mapContainsKey(
					this.railwayNodes, railwayLink.getEndNodeId(),
					"railwayNodes");
	...
	private void buildRailwayLines() {
		for (RailwayLine railwayLine : this.railwayLines.values()) {
			final List<RailwayLink> railwayLinks = new ArrayList<>(railwayLine
					.getRailwayLinkIds().size());
			for (String linkId : railwayLine.getRailwayLinkIds()) {
				final RailwayLink railwayLink = Ensure.mapContainsKey(
						this.railwayLinks, linkId, "railwayLinks");
				railwayLinks.add(railwayLink);
				Ensure.propertyIsNull(railwayLink.getRailwayLine(),
						railwayLink, "railwayLine");
				railwayLink.setRailwayLine(railwayLine);
				railwayLink.setRailwayLineId(railwayLine.getId());
				railwayLink
						.setRailwayLineCode(railwayLine.getRailwayLineCode());
				railwayLink.setRailwayLineGeographicalName(railwayLine
						.getGeographicalName());
			}
	...
	private void buildMarkerPosts() {
		for (MarkerPost markerPost : this.markerPosts.values()) {
			...
			final RailwayLine railwayLine = Ensure.mapContainsKey(
					this.railwayLines, railwayLineId, "railwayLines");
			markerPost.setRailwayLine(railwayLine);
			markerPost.setRailwayLineCode(railwayLine.getRailwayLineCode());
			markerPost.setRailwayLineGeographicalName(railwayLine
					.getGeographicalName());
			...
			for (RailwayLink railwayLink : railwayLine.getRailwayLinks()) {
				final List<Double> railwayLinkCoordinates = railwayLink
						.getCoordinates();
				for (int index = 0; index < railwayLinkCoordinates.size() - 2; index += 2) {
					final List<Double> subList = railwayLinkCoordinates
					...
					final double distance = DistanceCalculator
							.calculateDistance(new double[] { a0, a1 },
									new double[] { b0, b1 }, new double[] { q0,
											q1 });
					distancesToLinks.put(distance, subList);
				}
			}
			final Entry<Double, List<Double>> firstEntry = distancesToLinks
					.firstEntry();
			{
				...
				Vector2D m = new Vector2D(markerPost.getCoordinates().get(0),
						markerPost.getCoordinates().get(1));
				Vector2D a = new Vector2D(segmentCoordinates.get(0),
						segmentCoordinates.get(1));
				Vector2D b = new Vector2D(segmentCoordinates.get(2),
						segmentCoordinates.get(3));
				...
				if (distance >= 1.0e-6) {
					System.out.println("Distance is greater than 1.0e-6:"
							+ distance);
				}
			}
		}
	...
	private void buildRailwayLinkSequences() {
		for (RailwayLinkSequence railwayLinkSequence : this.railwayLinkSequences
				.values()) {
			final List<RailwayLink> railwayLinks = new ArrayList<>(
					railwayLinkSequence.getRailwayLinkIds().size());
			for (String linkId : railwayLinkSequence.getRailwayLinkIds()) {
				final RailwayLink railwayLink = Ensure.mapContainsKey(
						this.railwayLinks, linkId, "railwayLinks");
				railwayLinks.add(railwayLink);
				Ensure.propertyIsNull(railwayLink.getRailwayLinkSequence(),
						railwayLink, "railwayLinkSequence");
				railwayLink.setRailwayLinkSequence(railwayLinkSequence);
				railwayLink.setRailwayLinkSequenceId(railwayLinkSequence
						.getId());
				final RailwayLine railwayLine = railwayLinkSequence
						.getRailwayLine();
				if (railwayLine == null) {
					railwayLinkSequence.setRailwayLine(railwayLink
							.getRailwayLine());
				} else {
					Ensure.propertyEquals(railwayLine, railwayLinkSequence,
							"railwayLine", railwayLink.getRailwayLine());
				}
			}
